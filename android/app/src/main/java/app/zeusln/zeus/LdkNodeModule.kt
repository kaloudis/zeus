...
class LdkNodeModule(reactContext: ReactApplicationContext) : ReactContextBaseJavaModule(reactContext) {
    ...
    @ReactMethod
    fun forceCloseChannel(userChannelId: String, counterpartyNodeId: String, reason: String, promise: Promise) {
        moduleScope.launch {
            try {
                val node = this@LdkNodeModule.node ?: throw Exception("Node not initialized")
                node.forceCloseChannel(userChannelId, counterpartyNodeId, reason.ifEmpty { null })
                withContext(Dispatchers.Main) {
                    promise.resolve(null)
                }
            } catch (e: Exception) {
                ...
                val paymentMap = Arguments.createMap().apply {
                    ...
                    when (val kind = payment.kind) {
                        is PaymentKind.Onchain -> {
                            ...
                            when (val status = kind.status) {
                                is ConfirmationStatus.Confirmed -> {
                                    putString("status", "confirmed")
                                    putDouble("confirmationHeight", status.height.toDouble())
                                    putDouble("confirmationTimestamp", status.timestamp.toLong().toDouble())
                                }

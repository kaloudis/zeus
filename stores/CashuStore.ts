...
export default class CashuStore {
    ...
    private mintSupportsMpp = (mintUrl: string): boolean => {
        ...
        const mintInfo =
            this.mintInfos[normalizedMintUrl] || this.mintInfos[mintUrl];
        const nuts: any = mintInfo?.nuts;

        return !!(nuts && (nuts[15] || nuts['15']));
    };
    ...
    private payLnInvoiceFromEcashMultiMint = async ({
        ...
        if (mintsBySmallestBalance.length === 0) {
            runInAction(() => {
                this.paymentError = true;
                this.paymentErrorMsg = localeString(
                    'stores.CashuStore.notEnoughFunds'
        ...
        let totalFeePaid = 0;
        const segmentPayments: CashuPayment[] = [];
        ...
        for (const { mintUrl } of mintsBySmallestBalance) {
            ...
            let mintBalance = normalizedBalances[mintUrl] || 0;
            if (mintBalance <= 0) {
                continue;
            }
            ...
            let quote = await this.createMeltQuoteCDK(mintUrl, paymentRequest, {
                mpp: { amount: initialAllocation }
            });

            let requiredFromMint = quote.amount + quote.fee_reserve;

            if (requiredFromMint > mintBalance) {
                const affordableAllocation = Math.max(
                    0,
                    Math.min(remainingAmount, mintBalance - quote.fee_reserve)
                );

                if (affordableAllocation <= 0) {
                    continue;
                }

                quote = await this.createMeltQuoteCDK(mintUrl, paymentRequest, {
                    mpp: { amount: affordableAllocation }
                });
                requiredFromMint = quote.amount + quote.fee_reserve;
            ...
            const meltResult = await CashuDevKit.melt(mintUrl, quote.id);
            const paidAmount = meltResult.amount || quote.amount;
            const meltState = (
                meltResult.state ||
                quote.state ||
                ''
            ...
            totalFeePaid += meltResult.fee_paid || 0;
            remainingAmount = Math.max(0, remainingAmount - paidAmount);
            mintBalance = Math.max(0, mintBalance - requiredFromMint);
            normalizedBalances[mintUrl] = mintBalance;
            ...
            const payment = new CashuPayment({
                ...
                meltResponse: {
                    quote: {
                        ...
                        expiry: quote.expiry,
                        payment_preimage: meltResult.preimage || null
                    },
                ...
                payment_preimage: meltResult.preimage || '',
                mintUrl
            });

            segmentPayments.push(payment);
        }
        ...
        runInAction(() => {
            this.lastMultiMintUsedMints = segmentPayments.map(
                (payment) => payment.mintUrl
            );

            if (!isDonationPayment) {
                const preimage = segmentPayments
    ...
    public getMultimintInfo = (): MintProgressInfo[] => {
        const availableMints = this.mintUrls.map((mintUrl) => {
            const mintName = this.mintInfos[mintUrl]?.name || mintUrl;
            return {
                mintUrl,
                mintName,
